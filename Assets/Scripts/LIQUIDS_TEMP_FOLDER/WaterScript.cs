...
public class WaterScript : LiquidScript
{
    protected override void UpdateLiquidPosition()
    {
        ...
        else if (flowDirection == 0)//if flow direction is left
        {
            if (!liquidManagerScript.CheckIfTile((ushort)(currentPos.x - 1), (ushort)currentPos.y))//check left
            {
                newPos = new Vector2((ushort)(currentPos.x - 1), (ushort)currentPos.y);
            }
            else if (!liquidManagerScript.CheckIfTile((ushort)(currentPos.x + 1), (ushort)currentPos.y))//check right
            {
                newPos = new Vector2((ushort)(currentPos.x + 1), (ushort)currentPos.y);
                flowDirection = 1;
            }
